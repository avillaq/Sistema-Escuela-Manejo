...
class CrearAdministradorSchema(Schema):
    nombre = fields.Str(
        ...
        validate=validate.Length(
            min=1,
            error="El nombre no tiene los caracteres suficientes."
        ),
    ...
    apellidos = fields.Str(
        ...
        validate=validate.Length(
            min=1,
            error="El apellido no tiene los caracteres suficientes."
        ),
    ...
    email = fields.Str(
        validate=email_opcional,
        error_messages={"invalid": "El correo electrónico no es válido."}
    )
