...
def generar_bloques(semanas_futuro=2):
    ...
    with app.app_context():
        ...
        if bloques_creados > 0:
            db.session.commit()
            logger.info(f"Se crearon {bloques_creados} bloques nuevos")
        else:
            logger.info("No se requirió crear bloques nuevos")
...
def limpiar_bloques_vacios():
    ...
    with app.app_context():
        ...
        if cantidad > 0:
            ...
            db.session.commit()
            logger.info(f"Se eliminaron {cantidad} bloques vacíos")
        else:
            logger.info("No hay bloques vacíos para eliminar")
